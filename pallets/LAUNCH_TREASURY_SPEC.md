...
- **D10 (2026-09-22):** `DefaultFeeRouting` is one entry **per fee tier** (`StorageMap<u32, FeeRouting, OptionQuery>`), so a 1 % pool may route what a 0.3 % pool cannot — without it a tier-10 pool routed the same ≤ 30 bps as tier 3 and the other 70 accrued to a permanently locked LP position, the dead weight §2.6 was written about. `set_default_fee_routing(fee_tier, protocol, creator, treasury)` validates against *that* tier with `is_valid_for` (`is_valid_default` is gone), and refuses a creator or treasury slice below `MIN_LAUNCH_FEE_TIER`, where no launch pool can exist. **Absent is not zero:** `routing_for_new_pool` fails a launchpad seed at an unconfigured tier with `NoDefaultFeeRouting` (FM-11, a deferred graduation, retried once governance sets the tier) rather than graduating a pool whose treasury would be fed nothing for its life; a `create_pool` pool at an unconfigured tier routes nothing, as before any default existed. Storage version 3 → 4 with a migration: the single pre-D10 value becomes tier 3's entry if it still fits under the floor, is dropped with a warning if it does not (a split nobody chose is worse than none, and none is visible), and tiers 1 and 10 are left for governance to set deliberately. Existing pools are untouched — routing is snapshotted at creation, so **tier-aware routing reaches only pools seeded after the upgrade**.
- ~~`set_default_fee_routing(protocol, creator, treasury)` validates with `is_valid_default`;~~ (pre-D10) `insert_new_pool` validates the snapshot against the actual tier (`InvalidFeeRouting`, which at seed time is a deferred graduation, FM-11). `routing_for_new_pool` folds `creator_bps` and `treasury_bps` into the pool for `create_pool` pools (no launch, no treasury).
...
2. **Bounty vs dust** — *answered, 2026-09-23, and the premise was wrong.* There is no dust problem: the recipient's ability to receive is `free + b ≥ ED`, not `b ≥ ED` (`pallet_balances/src/impl_fungible.rs`, `can_deposit`), and a caller that just paid for the extrinsic exists. The implemented `b ≥ ED` test suppressed the payment without suppressing the work — 793 compounds on the dev chain, `bounty: 0` on every one — while 100 % of `realised` went to the buyback. §6.4 now asks `can_deposit` instead, and no threshold is introduced: a threshold on the *sale* would strand sub-floor LNRG in any retired treasury for good, since `do_retire` freezes `lnrg_accrued` and §6.4's close-out requires it to be zero.
...
4. ~~**The routing bound is `routed ≤ tier × 10`**~~ — *reverted by D10 (2026-09-22): the floor §2.6 asked for is back, as `pool_floor_bps(tier) = min(MIN_POOL_BPS (10), tier × 10 / 2)`, so tier 1 keeps 5 (which is what the tier-1 protocol slice was sized against, the reason the floor was dropped) and tiers 3 and 10 keep 10, leaving 20 and 90 bps routable. `is_valid_for` carries the floor and is what every creation path checks; the weaker `fits_tier` (`routed ≤ tier × 10`) is the arithmetic invariant `try_state` keeps, because a pool created under the old bound holds its snapshot for life and must not trip a later policy.* The original entry read: **The routing bound is `routed ≤ tier × 10`** (§7.1), i.e. a pool may route its whole tier, rather than `tier × 10 − 10`. D4's rationale was "the pool keeps ≥ 0", and a `create_pool` pool at tier 1 with today's 5-bps protocol slice needs the smaller reserve; the default is separately required to fit the protocol slice into tier 1 and all three into tier 3.
...
11. **A from-genesis chain does not run `FundVault`** (`frame_system` writes `LastRuntimeUpgrade` at genesis). See §9.6: without the ED buffer the retirement cannot close — the dev-chain run stranded 2.187 VTRS of `pending_burn` on slice 19 with `Token(Frozen)`. On a chain that receives the pallet by upgrade the migration runs and none of this arises; the §9.6 fix (applied) makes the pallet not depend on which path it took.
12. **Where pallet code lives, and the one rule a later session must not get wrong.** *Superseded on 2026-09-17, when the Foundation split #100 into three: the benchmark repairs against `develop`, the pallets into `power-plant-experimental`, and #100 itself reduced to testnet wiring that depends on experimental by a pinned commit.* *Updated 2026-09-21: experimental PR #1 is merged — `Vitreus-Foundation/power-plant-experimental` `main` at `254ca48` is the authoritative copy of `pallets/vitreus-dex` and `pallets/launchpad`, migrations included; `pallets/launch-treasury` is authoritative on PR #2 (`Bison1330:pallets/launch-treasury`, rebased onto that `main`) until it merges, then on `main` too. The benchmark repairs merged as power-plant #101. What that changes, concretely:*
    ...
    - **A consumer's own tree holds only what is not pallet code:** `construct_runtime!` wiring, `Config` impls, the runtime adapters (`EnergyGenerationStaking`, the broker exchange adapter), the runtime's `Unreleased` list (which names the pallet's `FundVault`), `spec_version`, and the fork's `dev-ops/preflight/` records. The fork's local `pallets/{vitreus-dex,launchpad,launch-treasury}` directories are deleted when its pin lands, and `design/launch-treasury` and the pallet copies on `pr/dex-launchpad` retire with them.
